...
K.TestLogMultiplier = 20
K.TestSamplesNum = 5000
...
if __name__ == '__main__':
    ...
    if not K.IsRecover:
        net = InceptionResNet1D(data_manager.classes_num, num_input_channels=1 if K.IOnly else 2, batch_size=K.BatchSize
                                **K.ModelSettings)
        ...
        net, optimizer = saver.restore(K.RecoverEpochID,
                                       model_cls=InceptionResNet1D, optimizer_cls=torch.optim.Adam)
        # ** If recover training, start at recover epoch
        start_epoch = K.RecoverEpochID + 1
    net.cuda()
    # * Start training
    for epochID in range(start_epoch, K.NumEpochs):
        ...
        for batch_ID, train_batch in enumerate(train_batches):
            ...
            if iteration % K.TrainLogInterval == 0 and iteration != 0:
                ...
                if (iteration / K.TrainLogInterval) % K.TestLogMultiplier == 0:
                    ...
                    process_bar.SkipMsg(
                        '({}/{}) train_loss: {}, train_accuracy: {}, test_loss: {}, test_accuracy: {}'.format(
                            batch_ID, batches_num - 1, train_loss, train_accuracy, test_loss, test_accuracy)
                        , logger)
                    # * Add compare scalar
                    ...
                    process_bar.SkipMsg(
                        'Train: ({}/{}) loss: {}, accuracy: {}'.format(batch_ID, batches_num - 1, train_loss, train_accuracy),
                        logger)
            # * Process Batch Data
